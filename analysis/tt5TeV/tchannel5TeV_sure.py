...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        lep_phi=ak.Array([False if len(item) == 0 else item[0] for item in l_sel.phi])
        lep_pt=ak.Array([False if len(item) == 0 else item[0] for item in l_sel.pt])
        ...
        fake_phi=ak.Array([False if len(item) == 0 else item[0] for item in l_fake.phi])
        fake_pt=ak.Array([False if len(item) == 0 else item[0] for item in l_fake.pt])
...
#######################################################################################################    Adding by myself extra cuts to replicate ATLAS selection
              ...
              mtw_fake=np.sqrt(2*fake_pt*met_pt*(1-np.cos(abs(fake_phi-met.phi))))
            
              
              cutmtwl=mtw>35
              cutmtw=ak.Array(cutmtwl)   #For adding the cut in Mw (me)
              cutmtwl_fake=mtw_fake>35
              cutmtw_fake=ak.Array(cutmtwl_fake) 
              
              
              cutmet_mtw=met_pt+mtw>70  #For adding cut in sum of MET and Mw (me)
              cutmet_mtw_fake=met_pt+mtw_fake>70 
              
              cutht=ht_var+lep_pt+met_pt>185  #For adding cut in Ht (me) Beware this is a different definition from our usual Ht
              cutht_fake=ht_var+fake_pt+met_pt>185
              
              
              if ch == 'e': mlb=GetMlb1b(e0,b0)
              if ch== 'm': mlb=GetMlb1b(m0,b0)
              #if ch in ['e_fake','m_fake']: mlb=GetMlb1b(lfake0,b0)
              ...
              print('antes',e0.pt)
              if ch in ['e','m']: print('leppt',getattr(eval(var),'pt'))
...
########################################################################################################                   Up to here the cut thing
              ...
              cuts      = [ch] + [lev] + ['metg15'] + ['METfilters']
              cutjets = GetCutJets(cuts, syst, met_pt, njets_var, nbtags_var)
              
              
              cut = (cutsel) & (cutjets) & (cutbeta) & (cutueta) & (cutdeltaeta) & (cutmtw) & (cutmet_mtw) & (cutht)& (cutmlb)#i've added some cuts here
              cut = np.array(cut, dtype=bool)												#main cut
              
              cut1 = (cutsel) & (cutjets) & (cutbeta) & (cutueta) & (cutdeltaeta) & (cutmtw) & (cutmet_mtw) & (cutht)#i've added some cuts here
              cut1 = np.array(cut1, dtype=bool)												#main cut
              
              if ch in ['e_fake','m_fake']: cut=np.array((cutsel)&(cutjets)& (cutbeta) & (cutueta) & (cutdeltaeta)& (cutmtw_fake) & (cutmet_mtw_fake) & (cutht_fake) & (cutmlb),dtype=bool) #Not sure what to do with cuts in these channels: they may affect (for sure) QCD calculation


              cutsnoMET = [ch] + [lev] + ['METfilters']
              cutjetsnomet = GetCutJets(cutsnoMET, syst, met_pt, njets_var, nbtags_var, nujets_var)
              cutnomet = (cutsel) & (cutjetsnomet) & (cutbeta) & (cutueta) & (cutdeltaeta)     #not sure if i have to stop at second cut
              cutnomet = np.array(cutnomet, dtype=bool)										#special cuts for met validations
              
              #if ch in ['e_fake','m_fake']: cutnomet=np.array((cutsel)&(cutjetsnomet),dtype=bool)#& (cutbeta) & (cutueta) & (cutdeltaeta),dtype=bool)  #Not sure what to do with cuts in these channels: they may affect (for sure) QCD calculation
